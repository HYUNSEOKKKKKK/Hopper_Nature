...
def tensorboard_launcher(directory_path):
    ...
    tb.configure(argv=[None, '--logdir', directory_path,'--port',str(6100)])
    url = tb.launch()
    print("[RAISIM_GYM] Tensorboard session created: "+url)
    webbrowser.open_new(url)


def load_param(weight_path, env, actor, critic, barrier_critic, estimator, optimizer, data_dir):
    if weight_path == "":
    ...
    barrier_critic.architecture.load_state_dict(checkpoint['barrier_critic_architecture_state_dict'])
