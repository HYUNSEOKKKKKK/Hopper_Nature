...
PYBIND11_MODULE(RAISIMGYM_TORCH_ENV_NAME, m) {
  py::class_<VectorizedEnvironment<ENVIRONMENT>>(m, RSG_MAKE_STR(ENVIRONMENT_NAME))
    ...
    .def("actorObserve", &VectorizedEnvironment<ENVIRONMENT>::actorObserve)
    .def("valueObserve", &VectorizedEnvironment<ENVIRONMENT>::valueObserve)
    .def("getTrueState", &VectorizedEnvironment<ENVIRONMENT>::getState)
    ...
    .def("setTerrain", &VectorizedEnvironment<ENVIRONMENT>::setTerrain)
    .def("setInitial", &VectorizedEnvironment<ENVIRONMENT>::setInitial)
    ...
    .def("getEstDim", &VectorizedEnvironment<ENVIRONMENT>::getEstDim)
