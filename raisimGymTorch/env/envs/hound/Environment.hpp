...
//    limitCOMpos_ << -0.04, 0.04; /// only enforced standingMode
    ...
    limitSmoothness2_ << -30,30; // actuator num * 10
...
//      rotConversion_ << 0.9393727,  0.0000000,  0.3428978, 0.0000000,  1.0000000,  0.0000000, -0.3428978,  0.0000000,  0.9393727; // 0.35 rad pitch rot
      ...
      /// 발의 collision body (sole 박스 "Foot/0" + 앞쪽 toe 박스 "Foot/1") 재질 지정.
      /// default 재질로 남으면 restitution 0 / 마찰 randomize 미적용이라 접촉이 비대칭이 됨.
      /// toe 는 sole 과 마찰은 같게, restitution 만 따로 주기 위해 별도 재질 -> reset() 에서 쌍 속성 지정
      for (auto& col : dhal_->getCollisionBodies()) {
          if (col.localIdx == dhal_->getBodyIdx("Foot")) {
              col.setMaterial((col.colObj->name == "Foot/1") ? "toe" : "rubber");
          }
      }
...
//    std::cout << "dhal_->getMass()[3]: " << dhal_->getMass()[3] << std::endl;
    ...
    /// rotation bias
    quat_.coeffs() << uniDist_(gen_)*0.03, uniDist_(gen_)*0.03, 0.0, 1.0; // max 0.084 rad -> 5 deg
    quat_.normalize();
    rotBiased_ = quat_;
  }
...
//      }
      ...
      /// Log Barrier - limit_barrier_smoothness
      relaxedLogBarrier(1.0, limitSmoothness2_(0), limitSmoothness2_(1),(pTarget_ - 2 * prevTarget_ + prevPrevTarget_).squaredNorm(),tempReward);
...
//              rot_.e().transpose() * ((edgePosWorld_.col(2)+edgePosWorld_.col(3))/2.0 - gc_.head(3)) - temp,/// relative edge pos (heel, toe)
 ...
 private:
  ...
  Eigen::Matrix<double,3,3> rotConversion_, rotBiased_;
  raisim::Mat<3,3> rot_;
